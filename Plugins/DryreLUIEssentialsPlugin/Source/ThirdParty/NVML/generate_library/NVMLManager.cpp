...
int nvGetGPUVRAMNVML(int Index, E_NV_VRAM_STATUS_NVML state)
{
	...
	if (state == E_NV_VRAM_STATUS_NVML::Free) {
    return static_cast<int>(memoryInfo.free / byteToMB);
} else if (state == E_NV_VRAM_STATUS_NVML::Used) {
    return static_cast<int>(memoryInfo.used / byteToMB);
} else if (state == E_NV_VRAM_STATUS_NVML::Total) {
...
std::string nvGetBoardPartNumberNVML(int Index)
{
	...
	return std::string(boardPartNumber, strnlen(boardPartNumber, NVML_DEVICE_PART_NUMBER_BUFFER_SIZE));
}
...
int nvGetGPUGetMemoryBusWidthNVML(int Index)
{
	...
	{
		// Handle NVML initialization error
		// HUE_LOG(LogTemp, Error, TEXT("Error initializing NVML."));
		return -1;
	}
	...
	{
		// Handle device retrieval error
		// HUE_LOG(LogTemp, Error, TEXT("Error getting GPU device"));
		return -1;  // or another appropriate value
	}
	...
	if (result != NVML_SUCCESS)
	{
		...
		// HUE_LOG(LogTemp, Error, TEXT("Error getting GPU device: %s"));
		return -1;
	}
...
std::string nvGetGPUGetNameNVML(int Index)
{
	...
	// Return the GPU name as std::string
	return std::string(name, strnlen(name, MaxNameLength));
}
...
int nvGetGPUGetCudaComputeCapabilityNVML(int Index)
{
	...
	if (result != NVML_SUCCESS)
	{
		...
		// HUE_LOG(LogTemp, Error, TEXT("Error getting CUDA compute capability: %s"), *FString(nvmlErrorString(result)));
		return -1;
	}
...
std::string nvGetGPUGetPowerStateNVML(int Index)
{
	...
	if (result != NVML_SUCCESS)
	{
		...
		return "Error getting GPU power state";
	}
	...
	*/
	...
	{
	case NVML_PSTATE_0:
		powerStateString = "Maximum Performance";
		break;
	case NVML_PSTATE_1:
		powerStateString = "Performance State 1";
		break;
	case NVML_PSTATE_2:
		powerStateString = "Performance State 2";
		break;
	case NVML_PSTATE_3:
		powerStateString = "Performance State 3";
		break;
	case NVML_PSTATE_4:
		powerStateString = "Performance State 4";
		break;
	case NVML_PSTATE_5:
		powerStateString = "Performance State 5";
		break;
	case NVML_PSTATE_6:
		powerStateString = "Performance State 6";
		break;
	case NVML_PSTATE_7:
		powerStateString = "Performance State 7";
		break;
	case NVML_PSTATE_8:
		powerStateString = "Performance State 8";
		break;
	case NVML_PSTATE_9:
		powerStateString = "Performance State 9";
		break;
	case NVML_PSTATE_10:
		powerStateString = "Performance State 10";
		break;
	case NVML_PSTATE_11:
		powerStateString = "Performance State 11";
		break;
	case NVML_PSTATE_12:
		powerStateString = "Performance State 12";
		break;
	case NVML_PSTATE_13:
		powerStateString = "Performance State 13";
		break;
	case NVML_PSTATE_14:
		powerStateString = "Performance State 14";
		break;
	case NVML_PSTATE_15:
		powerStateString = "Minimum Performance";
		break;
	case NVML_PSTATE_UNKNOWN:
		powerStateString = "Unknown Performance";
		break;
	default:
		powerStateString = "Maximum Performance";
		break;
